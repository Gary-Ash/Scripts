...
our @plistKeysToDelete = (
    ...
    "ApplicationAutoSaveState",										  "ApplicationSleepState",
);
...
our @itemsToDelete = (
    ...
    ["$HOME/Library/Containers/com.barebones.bbedit/Data/Library/BBEdit",                                                                 0],
    ...
    if ($plist && $$plist) {
        ...
        for (my $index = 0; $index < $items; ++$index) {
            ...
            if (rindex($key, "InstaprojectWindowSavedBounds", 0) != -1 ||
            	rindex($key, "CopiedPreferencesToNewKeys_", 0) != -1   ||
...
#*****************************************************************************************
nohup osascript <<"SHUTDOWN" &; disown
try
