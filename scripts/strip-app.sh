...
# Hand the deferral to the shared logger: every severity line flushes first.
LOG_HOOK="flush_pending"
LOG_INDENT="  "
LOG_INDENT_VERBOSE="    "
...
#
# shellcheck disable=SC2016  # the python program is single quoted on purpose
parse_seal() {
	local root="$1" plist
...
for line in sys.stdin:
    match = re.search(r"<key>(.*)</key>", line)
    ...
    if "<dict>" in line:
        ...
        continue
    if "</dict>" in line:
        if depth == 2 and key is not None:
            print(("cdhash" if cdhash else "content") + "\t" + key)
        depth -= 1
        continue
    if depth == 2 and "<key>cdhash</key>" in line:
        cdhash = True
    if depth == 1 and key is not None and "<data>" in line:
        print(f"content\t{key}")
'
}
...
# 0xcafebabe/0xcafebabf, so one python pass reads four bytes from each candidate and
...
for path in sys.argv[1:]:
    ...
    if len(magic) != 4:
        continue
    if struct.unpack(">I", magic)[0] in (0xCAFEBABE, 0xCAFEBABF):
...
thin_binary() {
	...
	if [[ ${APPLY} == true ]]; then
		cat "${tmp}" >"${file}"
		log_verbose "thinned ${file#"${APP}"/} ($((before / 1024)) KB -> $((after / 1024)) KB, dropped ${dead})"
	else
		log_verbose "$(action_prefix)thin ${file#"${APP}"/} ($((before / 1024)) KB, dropping ${dead})"
	fi
...
#
# The targets are resolved before anything is deleted, in one python pass rather than a
...
for link in sys.stdin.buffer.read().split(b"\0"):
    link = os.fsdecode(link)
    if not link:
        continue
    try:
    ...
    # Cwd::abs_path resolves every symlinked component but tolerates a missing final
    # one, so a dangling link is still pinned as long as its parent directory is there.
    # A loop is a genuine failure and drops the link, which is what abs_path did.
    try:
        print(os.path.realpath(target, strict=True))
        continue
    except OSError as error:
        if error.errno != errno.ENOENT:
            continue
    try:
        parent = os.path.realpath(os.path.dirname(target), strict=True)
    except OSError:
        continue
    print(os.path.join(parent, os.path.basename(target)))
...
for key, body in re.findall(
    r"<key>([^<]*\.lproj/)</key>\s*<dict>(.*?)</dict>", sys.stdin.read(), re.S
):
    if "Base" in key:
        continue
    if re.search(r"<key>optional</key>\s*<true/>", body):
        ok = True
...
strip_lproj_dir() {
	...
	if [[ ${matched} == true ]]; then
		...
		log_verbose "no system-language, Base or English resource in ${parent#"${APP}"/} - left intact"
	...
	for child in "${parent}"/*.lproj; do
		...
		if is_pinned "${child}"; then
			log_verbose "kept ${child#"${APP}"/} (a symlink elsewhere in the bundle points into it)"
			continue
		fi

		if ! lproj_removable "${APP}" "${child}"; then
			log_verbose "kept ${child#"${APP}"/} (not covered by an optional seal rule)"
			continue
		fi
		...
		log_verbose "$(action_prefix)remove ${child#"${APP}"/} (${size} KB)"
...
strip_gettext_root() {
	...
	for child in "${root}"/*; do
		...
		if ! gettext_removable "${child}"; then
			log_verbose "kept ${child#"${APP}"/} (sealed as a mandatory resource)"
			continue
		fi
...
verify_and_rollback() {
	...
	for _ in 1 2 3; do
		...
		while IFS= read -r path; do
			...
			if stage_restore "${path}"; then
				...
				log_info "kept as it was: ${path#"${app}"/} (codesign seals it by content)"
			fi
		done < <(printf '%s\n' "${output}" |
	...
	log_warn "${app##*/}: rolling back every change - codesign still refuses the bundle"
	stage_restore_all
	verify_bundle "${app}"
}
...
process_app() {
	...
	if [[ ${DO_THIN} == true ]]; then
		while IFS= read -r -d '' record; do
			file="${record#*$'\t'}"
			if [[ ${record} == skip$'\t'* ]]; then
				log_verbose "left fat: ${file#"${APP}"/} (sealed as a resource)"
				continue
			fi
	...
	if [[ ${APPLY} == true && $((n_thinned + n_lang_removed)) -gt 0 ]]; then
		if ! verify_and_rollback "${app}"; then
			log_error "${app##*/}: could not be returned to a verifying state - restore it from a backup"
			return 1
		fi
	fi
	...
	log_info "binaries thinned      : ${n_thinned}"
	[[ ${n_thin_skipped} -gt 0 ]] && log_info "binaries left fat     : ${n_thin_skipped} (sealed by content)"
	log_info "localizations removed : ${n_lang_removed}"
	[[ ${n_lang_kept_intact} -gt 0 ]] && log_info "localizations kept    : ${n_lang_kept_intact} set(s) with no safe fallback"
	if [[ ${APPLY} == true ]]; then
		log_info "size                  : ${before} KB -> ${after} KB"
	else
		log_info "reclaimable           : $(((bytes_thin + bytes_lang) / 1024)) KB (intel $((bytes_thin / 1024)) KB, lang $((bytes_lang / 1024)) KB)"
	fi
...
damage_report() {
	...
	if [[ ${gone} -eq ${count} ]]; then
		log_info "${count} sealed resource(s) deleted - a localization strip that ignored"
		log_info "the seal"
	elif [[ ${machos} -eq ${count} ]]; then
		log_info "${count} Mach-O file(s) were thinned despite being sealed by content"
	elif [[ $((gone + machos)) -gt 0 ]]; then
		log_info "${count} sealed item(s) altered: ${machos} Mach-O thinned, ${gone} deleted,"
		log_info "$((count - gone - machos)) otherwise rewritten - part of this is a strip"
		log_info "that ignored the seal"
	else
		log_info "${count} sealed file(s) altered, none of them Mach-O - this is not"
		log_info "stripping damage; something else rewrote them"
	fi

	log_info "first: ${first#"${app}"/}"
	if [[ ${LOG_VERBOSE} == true ]]; then
		while IFS= read -r line; do
			[[ -n ${line} ]] || continue
			verb="${line%%$'\t'*}"
			path="${line#*$'\t'}"
			log_verbose "${verb}: ${path#"${app}"/}"
		done < <(damaged_paths "${app}")
	fi
...
repair_advice() {
	...
	if [[ ! -d "${app}/Contents/_MASReceipt" ]]; then
		log_info "repair: reinstall from the vendor - re-download and replace the bundle"
		return 0
	fi

	adam="$(mdls -raw -name kMDItemAppStoreAdamID "${app}" 2>/dev/null || true)"
	log_info "repair: App Store app - delete it and reinstall.  Re-signing is not an option;"
	log_info "        an ad-hoc signature drops the identifiers the sandbox depends on."
	if [[ -n ${adam} && ${adam} != "(null)" ]]; then
		if command -v mas >/dev/null 2>&1; then
			log_info "        mas install ${adam}"
		else
			log_info "        open \"macappstore://apps.apple.com/app/id${adam}\""
			log_info "        (or brew install mas, then mas install ${adam})"
		fi
	else
		log_info "        no App Store id recorded - reinstall from the Purchased list"
	fi
	return 0
}
...
prime_sudo() {
	...
	if [[ -r ${SUDO_HELPER} ]]; then
		...
		log_warn "${SUDO_HELPER} not found - falling back to an interactive prompt"
	fi
...
main() {
	...
	if [[ ${#TARGETS[@]} -eq 0 ]]; then
		...
		if [[ ${#TARGETS[@]} -eq 0 ]]; then
			log_error "no app bundles found in ${DEFAULT_SCAN_ROOT}"
			exit 1
		fi
	fi
	...
	if [[ ${MODE} == "strip" && ${APPLY} == true && ${EUID} -ne 0 ]] && needs_root; then
		log_line "re-running under sudo to modify root-owned bundles..."
		if ! prime_sudo; then
	...
	if [[ ${MODE} != "strip" ]]; then
		log_line "checking ${#TARGETS[@]} bundle(s) for a broken seal"
		for app in "${TARGETS[@]}"; do
			check_app "${app}" || rc=1
		done
		log_line ""
		if [[ ${total_apps} -eq 0 ]]; then
			log_line "every bundle verifies"
		else
			log_line "${total_apps} damaged bundle(s)"
			[[ ${MODE} == "check" ]] && log_line "run again with --repair for the remedy"
		fi
		return "${rc}"
	fi
	...
	if [[ ${#TARGETS[@]} -gt 1 ]]; then
		log_line ""
		if [[ ${APPLY} == true ]]; then
			log_line "total: ${total_apps} apps processed, $((total_bytes / 1024)) KB reclaimed"
		else
			log_line "total: ${total_apps} apps scanned, $((total_bytes / 1024)) KB reclaimable"
		fi
	fi

	[[ ${APPLY} == true ]] &&
		log_line "note: an app update replaces the bundle - re-run this afterwards."
