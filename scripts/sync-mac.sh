...
check_requirements() {
	...
	[[ ${#missing[@]} -eq 0 ]] || log_die "required command(s) not found: ${missing[*]}"
}
...
report_output() {
	...
	log_info "${description} failed${output:+ - ${output//$'\n'/; }}"
	case "${output}" in
		*"unexpected end of file"* | *"Operation not permitted"* | *"Permission denied"*)
			log_info "hint: sshd on ${target_system#*@} may need Full Disk Access - System Settings > Privacy & Security > Full Disk Access"
			;;
	esac
}
...
sync_directories() {
	...
	local directories_to_sync=(
		...
		"$HOME/Library/Containers/com.barebones.bbedit"
	)
	...
	for dir in "${directories_to_sync[@]}"; do
		if [[ ! -d ${dir} ]]; then
			log_info "skipping ${dir} - not a directory on this host"
			continue
		fi
	...
	for file in "${files_to_sync[@]}"; do
		if [[ ! -f ${file} ]]; then
			log_info "skipping ${file} - not present on this host"
			continue
		fi
...
sync_custom_icons() {
	...
	if [[ ! -d ${dir} ]]; then
		log_info "custom icon scan skipped - ${dir} is not a directory"
		return 1
	fi
	...
	if [[ -n ${script} ]]; then
		if ! printf '%s' "${script}" |
			...
			log_info "custom icon restore for ${dir} failed on ${target_system}"
			return 1
		fi
	fi
...
sync_mail_archive() {
	...
	local pref_files=(
		...
		"$HOME/Library/Containers/com.apple.mail/Data/Library/Preferences/com.apple.mail.plist"
	)

	if [[ ! -d ${mail_dir} ]]; then
		log_info "mail archive skipped - ${mail_dir} is not a directory on this host"
		return 1
	fi
...
sync_ruby_gems() {
	...
	if [[ ! -d /opt/venv/ruby ]]; then
		log_info "ruby gem sync skipped - /opt/venv/ruby not found on this host"
		return 1
	fi
	if ! eval "${rbenv_init}"; then
		log_info "rbenv initialization failed on this host"
		return 1
	fi
	if ! command -v gem >/dev/null 2>&1; then
		log_info "gem command not found after rbenv initialization"
		return 1
	fi

	if ! host_gem_list="$(gem list | sort)"; then
		log_info "unable to read the gem list on this host"
		return 1
	fi
	if ! target_gem_list="$(SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c '${rbenv_init} && gem list | sort'")"; then
		log_info "unable to read the gem list on ${target_system}"
		return 1
	fi

	# An empty host list would mark every gem on the target for removal
	if [[ -z ${host_gem_list} ]]; then
		log_info "host gem list is empty - refusing to sync gems to ${target_system}"
		return 1
	fi
	...
	if [[ -n ${gems_to_remove} ]]; then
		if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c '${rbenv_init} && gem uninstall -aIx ${gems_to_remove}'"; then
			log_info "gem uninstall failed on ${target_system}"
			status=1
		fi
	fi
	if [[ -n ${gems_to_install} ]]; then
		if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c '${rbenv_init} && gem install --force ${gems_to_install}'"; then
			log_info "gem install failed on ${target_system}"
			status=1
		fi
	fi
...
sync_pip_packages() {
	...
	if [[ ! -r /opt/venv/python3/bin/activate ]]; then
		log_info "pip package sync skipped - /opt/venv/python3 not found on this host"
		return 1
	fi
	if ! source /opt/venv/python3/bin/activate; then
		log_info "unable to activate the python virtual environment on this host"
		return 1
	fi
	if ! command -v pip3 >/dev/null 2>&1; then
		log_info "pip3 command not found after activating the virtual environment"
		return 1
	fi

	if ! host_freeze="$(pip3 list --format=freeze | sort)"; then
		log_info "unable to read the pip package list on this host"
		return 1
	fi
	if ! target_freeze="$(SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c '${venv_init} && pip3 list --format=freeze | sort'")"; then
		log_info "unable to read the pip package list on ${target_system}"
		return 1
	fi

	# An empty host list would mark every package on the target for removal
	if [[ -z ${host_freeze} ]]; then
		log_info "host pip package list is empty - refusing to sync packages to ${target_system}"
		return 1
	fi
...
sync_homebrew_packages() {
	...
	if ! host_formulae="$(brew list --formula | sort)"; then
		log_info "unable to read the Homebrew formula list on this host"
		return 1
	fi
	if ! target_formulae="$(SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c 'brew list --formula | sort'")"; then
		log_info "unable to read the Homebrew formula list on ${target_system}"
		return 1
	fi

	# An empty host list would mark every formula on the target for removal
	if [[ -z ${host_formulae} ]]; then
		log_info "host Homebrew formula list is empty - refusing to sync formulae to ${target_system}"
		return 1
	fi
	...
	if [[ -n ${casks_to_remove} ]]; then
		if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c 'brew uninstall -q --cask ${casks_to_remove} >/dev/null 2>&1'"; then
			log_info "Homebrew cask uninstall failed on ${target_system}"
			status=1
		fi
	fi
	if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c 'brew install -q --cask $(echo "${host_casks}" | tr '\n' ' ') >/dev/null 2>&1'"; then
		log_info "Homebrew cask install failed on ${target_system}"
		status=1
	fi
...
sync_npm_packages() {
	...
	if ! host_packages="$(npm list -g --depth=0 --parseable | tail -n +2 | sed 's#.*/node_modules/##' | sort)"; then
		log_info "unable to read the global npm package list on this host"
		return 1
	fi
	if ! target_packages="$(SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c 'npm list -g --depth=0 --parseable | tail -n +2 | sed \"s#.*/node_modules/##\" | sort'")"; then
		log_info "unable to read the global npm package list on ${target_system}"
		return 1
	fi

	# An empty host list would mark every global package on the target for removal
	if [[ -z ${host_packages} ]]; then
		log_info "host npm package list is empty - refusing to sync packages to ${target_system}"
		return 1
	fi
	...
	if [[ -n ${packages_to_remove} ]]; then
		if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c 'npm uninstall -g --silent ${packages_to_remove}'"; then
			log_info "npm global uninstall failed on ${target_system}"
			status=1
		fi
	fi
	if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "zsh -l -c 'PUPPETEER_SKIP_DOWNLOAD=1 npm install -g --silent $(echo "${host_packages}" | tr '\n' ' ')'"; then
		log_info "npm global install failed on ${target_system}"
		status=1
	fi
...
sync_custom_apps() {
	...
	if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "mkdir -p ${staging_dir}"; then
		log_info "unable to create the staging directory ${staging_dir} on ${target_system}"
		return 1
	fi
	...
	for app in "${apps_to_sync[@]}"; do
		...
		if [[ -d ${app_path} ]]; then
			...
			log_info "skipping ${app} - not present locally at ${app_path}"
		fi
	done

	if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" "rm -rf ${staging_dir}"; then
		log_info "unable to remove the staging directory ${staging_dir} on ${target_system}"
		status=1
	fi
...
save_bbedit_window_placement() {
	...
	if ! SSHPASS="${sudo_password}" sshpass -e ssh "${target_system}" \
		...
		log_info "BBEdit window placement backup failed on ${target_system}"
		return 1
	fi
...
REMOTE
	then
		log_info "BBEdit window placement restore failed on ${target_system}"
		return 1
	fi
...
main() {
	...
	if [[ -z ${current_host} ]]; then
		log_die "unable to determine the name of this host"
	fi
	...
	[[ ${known_host} -eq 1 ]] || log_warn "this host (${current_host}) is not in the sync list"

	sudo_password="$(get_sudo_password)"
	[[ -n ${sudo_password} ]] || log_warn "the sudo password is empty - ssh and remote sudo may fail"
	...
	for system in "${systems_to_sync[@]}"; do
		if [[ ${system} != "${current_host}" ]]; then
			...
			if ! target_reachable "${target_system}"; then
				log_error "${target_system} is unreachable - skipping"
				continue
			fi
	...
	[[ ${synced_count} -gt 0 ]] || log_warn "no target systems were synced"

	if [[ ${LOG_FAILURES} -gt 0 ]]; then
		log_info "completed with ${LOG_FAILURES} failure(s)"
		return 1
	fi
